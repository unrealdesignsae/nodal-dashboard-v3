
const { sheets } = require('@googleapis/sheets');
const { JWT } = require('google-auth-library');

async function main() {
  const auth = new JWT({
    email: "nodal-v2@nodal-dahsboard.iam.gserviceaccount.com",
    key: "-----BEGIN PRIVATE KEY-----\\nCaktxgXR27tU23XiibAS+Lh4\n-----END PRIVATE KEY-----\n",
    scopes: ['https://www.googleapis.com/auth/spreadsheets.readonly'],
  });
  const client = sheets({ version: 'v4', auth });
  
  // First get sheet metadata to see actual sheet names
  const meta = await client.spreadsheets.get({ spreadsheetId: "1iFUyaMhrA_HutzpgV88imc9peWfH_4ovyjLIFj33E-U" });
  console.log("Sheet tabs:");
  for (const s of meta.data.sheets) {
    console.log(" -", JSON.stringify(s.properties.title), "gid:", s.properties.sheetId);
  }
  
  // Try OVERVIEW without quotes
  const r = await client.spreadsheets.values.get({
    spreadsheetId: "1iFUyaMhrA_HutzpgV88imc9peWfH_4ovyjLIFj33E-U",
    range: "OVERVIEW!A1:Z10",
  });
  console.log("OVERVIEW rows:", r.data.values?.length ?? 0);
  console.log("First row:", JSON.stringify(r.data.values?.[0]));
  console.log("Second row:", JSON.stringify(r.data.values?.[1]));
}
main().catch(e => console.error(e.message));
